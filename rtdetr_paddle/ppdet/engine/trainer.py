...
from ppdet.utils.json_process import modify_bbox_json_imid
        ...
        if visualize:
            ...
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    image = visualize_results(image, bbox_res, mask_res, segm_res, keypoint_res,
                        pose3d_res, int(im_id), catid2name, draw_threshold)
                    
                    # 输出大于阈值框信息
                    # for number_bbox in bbox_res:
                    #     if number_bbox["score"] > draw_threshold:
                    #         print(number_bbox)
            ...
            # ---------------------------------------------------------------------------------------------------------------------
            # 预测json文件加入图片名称以及类别名称
            with open(os.path.join(output_dir, "bbox.json"), "r") as f:
                predict_result = json.load(f)
            for ann in predict_result:
                ann['file_name'] = os.path.basename(imid2path[ann['image_id']])
