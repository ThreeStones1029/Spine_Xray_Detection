...
def main(images_folder, output_folder):
    images_path1 = glob(os.path.join(images_folder, '*.png'))
    images_path2 = glob(os.path.join(images_folder, '*.bmp'))
    ...
    for image_path in images_path:
        ...
        he_eq_img.save(os.path.join(output_folder, filename + "_HE.png"))

        # AHE
        ahe_eq_img = icter.enhance_contrast(img, method = "AHE", window_size = 32, affect_size = 16)
        ahe_eq_img.save(os.path.join(output_folder, filename + "_AHE.png"))

        # CLAHE
        clahe_eq_img = icter.enhance_contrast(img, method = "CLAHE", blocks = 8, threshold = 10.0)
        clahe_eq_img.save(os.path.join(output_folder, filename + "_CLAHE.png"))

        # Local Region Stretch
        lrs_eq_img = icter.enhance_contrast(img, method = "Bright")
        lrs_eq_img.save(os.path.join(output_folder, filename + "_Bright.png"))
...
if __name__ == "__main__":
    # read image
    # img = Image.open("datasets/Final_semantic_Label_Modify/test/gcm_1.png")
    # print(np.array(img).shape)
    ...
    # clahe_eq_img.save("infer_output/1.jpg")

    # # Local Region Stretch
    # lrs_eq_img = icter.enhance_contrast(img, method = "Bright")
    # icter.plot_images(img, lrs_eq_img)
    main("datasets/All_datasets/All_X_Ray/images", "datasets/All_datasets/All_X_Ray/images_preprocessed")
